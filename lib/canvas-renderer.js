...
(() => {
  ...
  const keyToAction = keyCode => {
    if (keyCode === 'Escape') return SpaceInvader.Action.PAUSE;
    if (keyCode === 'ArrowLeft') return SpaceInvader.Action.MOVE_LEFT;
    if (keyCode === 'ArrowRight') return SpaceInvader.Action.MOVE_RIGHT;
    if (keyCode === 'Space') return SpaceInvader.Action.FIRE;
    return SpaceInvader.Action.DO_NOTHING;
  };

  const noEscKeyCode = val => val !== 'Escape';
  ...
  const startGame = () => {
    ...
    (function goNextFrame(previousState) {
      ...
      if (KEEP_STATES) states.push(previousState);
      ...
      const tmpKeyPool = keyPool.filter(noEscKeyCode);
