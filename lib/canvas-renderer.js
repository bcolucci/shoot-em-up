...
(() => {
  ...
  const startGame = () => {
    ...
    const makeAction = (keyCode, previousState) => {
    ...
    (function goNextFrame(previousState) {
      ...
      if (!keyPool.length)
        lastState = makeAction(null, lastState);
      else {
        keyPool.splice(0, keyPool.length).forEach(keyCode => {
          lastState = makeAction(keyCode, lastState);
        });
      }

      setTimeout(() => goNextFrame(lastState), FRAME_TIMEOUT);
